...
<Field title="schema" type="SchemaTransformer<T> | ZodSchema | JoiSchema | StandardSchema | undefined">
  - Default: `undefined`
  ...
  The schema is optional, but it is recommended to use it to ensure the configuration is valid and has the expected types.

  Supported schemas:
    - Zod v3
    - Zod v4
    - Joi
    - A schema that implements the [Standard Schema](https://standardschema.dev/) interface.
    - [Custom schemas](../guide/plugins.mdx#custom-schemas)
