...
The convert it back to the expected type. Make sure you configure your schema to coerce the types to the expected type.

With zod, you can easily do this using `z.coerce.number()`. For boolean you will have to use a helper function `zodCoercedBoolean` to coerce the string to a boolean. This is because zod does not have proper built-in coercion for booleans.
