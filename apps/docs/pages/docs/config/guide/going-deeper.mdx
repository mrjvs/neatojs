...
With Zod, you can easily do this using `z.coerce.number()` and `z.stringbool()`.
...
z.object({
  ...
  useStrictHeaders: z.stringbool().default(false),
})
```

<Warning>
For boolean values **in Zod v3** you will have to use a helper function provided by the library `zodV3CoercedBoolean` to coerce the string to a boolean. This is because Zod V3 does not have proper built-in coercion for booleans.
</Warning>
