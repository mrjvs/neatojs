...
<Field title="options" type="SiteOptions" required>
  ...
  <Field.Properties defaultOpen>
    ...
    <Field title="navigation" type="Array<LinkComponent | SeperatorComponent | CustomComponentComponent>">
      List of items shown in the top navigation bar. You can only specify `seperator()`, `link()` and `component()`.
    ...
    <Field title="meta" type="NextSeoProps | ((pageMeta: MetaTagPageMeta) => ReactNode)">
      ...
      Or you can pass in a function to render your own meta tags. Note that you do **have** to use [NextSeo](https://github.com/garmeeh/next-seo) to use this method.

      Read more about how to use this on [this page](../../guides/config/seo.mdx).
    </Field>
    <Field title="github" type="string">
      A repository identifier, if specified an github widget with the star count and fork count is displayed on the right side of the header.
    ...
    <Field title="logo" type="object">
      ...
      <Field.Properties>
        ...
        <Field title="to" type="string">
          Link to make the logo redirect towards of clicked. If not specified, the logo is not clickable
        </Field>
      </Field.Properties>
    </Field>
    <Field title="layout" type="string">
      The default layout to show for all pages associated with this site. Defaults to the layout with ID `default`.
    </Field>
    ...
    <Field title="contentFooter" type="object">
      ...
      Specifying options here or not will not influence if it is shown, check [the footer page](../../guides/advanced/footer.mdx) for more info.
