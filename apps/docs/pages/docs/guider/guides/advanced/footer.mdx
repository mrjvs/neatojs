...
site('site-a', {
  contentFooter: {
    ...
    // edit repository base must point to the folder that the guider project is in. (same as package.json)
...
The page footer can only hold text. However it can be very easy to replace the footer with something custom. Read more about [customizing layout](/docs/guider/guides/advanced/customizing-layout).
...
**Page footer is disabled by default.** It can be disabled even if you have the settings configured.
You can enabled it layout settings level. Read more about [layout settings](/docs/guider/guides/config/theming#introduction-to-layout-settings) if you want.
